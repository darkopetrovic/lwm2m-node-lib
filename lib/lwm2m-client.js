...
function startListener(deviceInformation, callback) {
    ...
    function extractListenerInfo(serverInfo, innerCallback) {
        debug('COAP Router started successfully');
    ...
    function handleListenerStarted(error, deviceInformation) {
        if (error) {
            debug('Failed to start COAP Router for client.');
        } else {
...
function createRegisterResponseHandler(deviceInformation, callback) {
    return function responseHandler(res) {
        if (res.code.match(/^2\.\d\d$/)) {
            debug('Registration oparation succeeded to host [%s] and port [%s] with code [%s]',
            ...
            debug('Registration failed with code: ' + res.code);
 ...
 * @param {String} binding               The binding used by the client
 */
function register(host, port, url, endpointName, binding, callback) {
    var rs = new Readable(),
        creationRequest =  {
            host: host,
            port: port,
            method: 'POST',
            pathname: ((url)? url : '') + '/rd',
            query: 'ep=' +  endpointName + '&lt=' + config.client.lifetime + '&lwm2m=' + config.client.version + '&b=' + binding
        },
    ...
    function sendRequest(payload, innerCallback) {
        ...
        rs.on('error', function(error) {
            debug('Error found trying to send registration request: %s', payload);
        ...
        req.on('error', function(error) {
            ...
            if (error.code === 'ENOTFOUND') {
                debug('Server not found [%s]', req.url.host);
                innerCallback(new errors.ServerNotFound(req.url.host));
            } else {
                debug('An error was found while trying to register a response listener');
                innerCallback(new errors.RegistrationError(error));
            }
        });
...
function updateRegistration(deviceInformation, callback) {
    var rs = new Readable(),
        creationRequest =  {
            ...
            query: 'lt=' + config.client.lifetime + '&lwm2m=' + config.client.version + '&b=' + deviceInformation.binding
        },
        agent = new coap.Agent({type: config.client.ipProtocol}),
        req = agent.request(creationRequest);

    debug('Update registration request:\n%s', JSON.stringify(creationRequest, null, 4));
    ...
    function sendRequest(payload, innerCallback) {
        ...
        rs.on('error', function(error) {
            debug('There was a connection error during update registration process: %s', error);
        ...
        req.on('error', function(error) {
            ...
            debug('Request error during update registration process: %s', error);
...
function unregister(deviceInformation, callback) {
    ...
    debug('Unregistration request:\n%s', JSON.stringify(creationRequest, null, 4));

    req.on('response', function(res) {
        debug('Unregistration response code:\n%s', res.code);
    ...
    req.on('error', function(error) {
        debug('There was an error during unregistration: %s', error);
