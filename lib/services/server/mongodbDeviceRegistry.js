...
var errors = require('../../errors'),
    ...
    informationReporting = require('./informationReporting'),
    context = {
...
function register(object, callback) {
    ...
    function mongoStore(innerCb) {
        ...
        deviceObj.binding = object.binding;
        deviceObj.name = object.name;
        deviceObj.type = object.type;
        deviceObj.objects = object.objects;
        deviceObj.creationDate = object.creationDate;
        deviceObj.lastSeen = object.lastSeen;
    ...
    getByName(object.name, function(error, result){
        if (!error && result) {
            /* If the device try to register before its lifetime expires and
             subscriptions still exists for the old device ID, an error may occurs. */

            informationReporting.cleanByDeviceId(result.id, function(){
...
function update(id, obj, callback) {
    getByParameter('id', id, function(error, objDAO) {
        if (error) {
            ...
            objDAO.binding = obj.binding;
            objDAO.name = obj.name;
            objDAO.type = obj.type;
            objDAO.objects = obj.objects;
            objDAO.creationDate = obj.creationDate;
            objDAO.lastSeen = obj.lastSeen;
        ...
        list(function(error, deviceList){
            if (!error && deviceList) {
                deviceList.forEach(function(device){
                    if (device.lifetime &&
                        new Date() - new Date(device.lastSeen) > Number(device.lifetime) * 1000) {
                        unregister(device.id, function(err, obj){
                            if (err) {
                                if(!obj){
                                    logger.error(context, 
                                        'Unregistration for device failded with error [%s]', err.code);
                                } else {
                                    logger.error(context, 
...
function init(config, callback) {
    ...
    //    config.deviceRegistry.host, config.deviceRegistry.port, config.deviceRegistry.db, callback);
}
