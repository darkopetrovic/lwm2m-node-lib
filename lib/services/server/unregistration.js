...
function endUnregistration(req, res) {
    return function (error, result) {
        if (error) {
            logger.error(context, 'Registration request ended up in error [%s] with code [%s]', error.message, error.code);

            res.code = error.code;
            res.end(error.message);
        } else {
            logger.info(context, 'Unregistration request ended successfully');
            res.code = '2.02';
            res.end('');
        }
    };
