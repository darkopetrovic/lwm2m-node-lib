...
function endUpdate(req, res) {
    return function (error, result) {
        if (error) {
            logger.error(context, 'Registration request ended up in error [%s] with code [%s]', error.name, error.code);

            res.code = error.code;
            res.end(error.message);
        } else {
            logger.info(context, 'Update registration request ended successfully');
...
function updateRegister(req, queryParams, obj, callback) {
    logger.debug(context, 'Updating device register with lifetime [%s] and address [%s].', queryParams.lt, req.rsinfo.address);
