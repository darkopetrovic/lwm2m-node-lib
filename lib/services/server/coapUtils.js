...
function clientIsOnline(req) {
    var d = deferred();

    if(req.queued){
        queued_request.push(req);
        console.log("\n Request placed in queue (waiting client with ID ["+req.deviceId+"] goes online).\n");
        var requestTimeout = setTimeout(function(){
            d.reject(queued_request);
        }, 120000);
        process.on('clientOnline'+req.deviceId, function(data) {
            clearTimeout(requestTimeout);
            d.resolve(queued_request);
        });
    } else {
        d.resolve();
    }
...
function sendRequest(request, callback) {
    ...
    clientIsOnline(request).then(function(reqq){
        ...
        if(reqq){
            reqq.splice(reqq.indexOf(request), 1);
        }
        ...
        callback(new errors.ClientConnectionError("QUEUED_REQUEST_TIMEOUT"));
    });
}
...
exports.init = init;
exports.queued_request = queued_request;