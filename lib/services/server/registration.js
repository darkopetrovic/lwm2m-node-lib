...
function storeDevice(queryParams, req, callback) {
    ...
    if(req.payload!=""){
        var objects = req.payload.toString().split(',');

        var objectsList = [];
        _.each(objects, function(o){
            var objectId = o.match(/<\/(.*)\/(.*)>/)[1];
            var objectInst = o.match(/<\/(.*)\/(.*)>/)[2];
            
            var mandRes = [];
            if(lwm2mid.getOid(objectId)){
                console.log(objectId);
                var obj = lwm2mid._defs.specificResrcChar[lwm2mid.getOid(objectId).key];

                for (var key in obj) {
                    if (obj.hasOwnProperty(key) && obj[key].mand == true) {
                        mandRes.push(lwm2mid._defs.uniqueRid[key]);
                    }
                }
            }
