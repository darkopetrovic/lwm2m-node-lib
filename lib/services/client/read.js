...
var async = require('async'),
    ...
    context = {
        op: 'LWM2MLib.ClientRead'
    },
    logger = require('../../debug')(context),
...
function cancelObservation(objUri, resourceId, callback) {
    if (observers[objUri + '/' + resourceId]) {
        ...
        logger.error(context, 'Tried to remove an unexistant observation on obj URI [%s] and resourceId [%s]',
            objUri, resourceId);
    }
...
function ResourceObserver(stream, resourceId, storedObject) {
    var data = {
        ...
        listener: function newModification(method, id, value) {
            if (method === 'setResource' && resourceId === id.toString()) {
                logger.debug(context, 'Sending data update to the server for object [%s]', data.uri);
                ...
                try {
                    ...
                    logger.debug(context, 'Tried to write to a closed stream. Cleaning the handler.');
                    cancelObservation(data.uri, resourceId, function() {
                        logger.info(context, 'Observation of [%s] cancelled', data.id);
                    });
                }
            }
        }
    ...
    function getLastMeasure() {
        objectRegistry.get(storedObject.objectUri, function (error, obj) {
            if (error) {
                logger.error(context, 'Error retrieving the last resource value');
            } else {
    ...
    objectRegistry.getAttributes(storedObject.objectUri + '/' + resourceId, function(error, attributes) {
        if (error) {
            logger.error(context, 'Couldn\'t find attributes for the [%s] resource URI. Falling back to value changed');
        }

        if (attributes || config.client.observe.period) {
            var period;
            if (attributes){
              period = attributes.pmax
            } else {
              period = config.client.observe.period;
            }

            if (period) {
                logger.debug(context, 'Creating a scheduled observation for [%d] miliseconds for object [%s]',
                  period, storedObject.objectUri);
            ...
            logger.debug(context, 'Creating an onchange subscription for object [%s]', storedObject.objectUri);
...
function handleRead(req, res, handler) {
    logger.debug(context, 'Handling incoming read request');
    ...
    extractUriInfo(req, res, function (error, objectUri, resourceId) {
        if (error) {
            ...
            if (_.some(req.options, _.matches({name: 'Accept', value: 'application/link-format'}))) {
                logger.debug(context, 'Retrieving simple response with static information for object: %s', objectUri);
                if (resourceId) {
                ...
                logger.debug(context, 'Creating observers for object URI: %s', objectUri);
                async.waterfall([
                    apply(objectRegistry.get, objectUri),
